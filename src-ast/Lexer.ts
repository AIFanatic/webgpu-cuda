 ...
 */
function isvariablename(src: string) {
...
export function tokenize(sourceCode: string): Token[] {
    ...
    while (src.length > 0) {
        ...
        // Handle single letter comparisons
        else if (src[0] == ">") tokens.push(token(src.shift(), TokenType.GreaterThan));
        else if (src[0] == "<") tokens.push(token(src.shift(), TokenType.LessThan));
